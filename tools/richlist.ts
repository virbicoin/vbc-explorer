...
const getTransactionAddresses = async (
  fromBlock: number,
  toBlock: number,
  retries = 3
): Promise<Set<string>> => {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.error(
        `❌ Error getting transaction addresses (attempt ${attempt}/${retries}):`,
        error
      );
      if (attempt < retries) {
...
const processAccountChunk = async (
  addresses: string[],
  blockNumber: number
): Promise<AccountData[]> => {
...
const updatePercentages = async (): Promise<void> => {
  try {
    ...
    for (let i = 0; i < accounts.length; i += batchSize) {
      ...
      await Account.bulkWrite(operations, { ordered: false });
      console.log(
        `📈 Updated percentages for batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(accounts.length / batchSize)}`
      );
    }
  } catch (error) {
...
const makeRichList = async (toBlock: number, blocks: number): Promise<void> => {
  ...
  try {
    ...
    if (addressesToProcess.length === 0) {
      ...
      if (updateCount > 0) {
        console.log(
          `📋 Processing ${newCount} new + ${updateCount} existing addresses (${cached} cached)`
        );
      } else {
    ...
    if (addressesToProcess.length > 0) {
      ...
      if (allAccountData.length > 0) {
        ...
        console.log(
          `📊 Total processed: ${processedCount} accounts (${newAddresses.length} new, ${existingAddressesToUpdate.length} updated)`
        );
      }
    }
