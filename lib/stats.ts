...
export async function getChainStats() {
  ...
  const { latestBlockDoc, latestBlock } = await getCachedData(
    ...
    async () => {
      try {
        const blockDoc = await db
          .collection('Block')
          .findOne({}, { sort: { number: -1 }, maxTimeMS: 30000 });
    ...
    300000
  ); // 5 minute cache for latest block (for t4g.small)

  // Calculate average block time from last 100 blocks with caching
  const avgBlockTime = await getCachedData(
    'avgBlockTime',
    async () => {
      try {
        ...
        if (recentBlocks && recentBlocks.length >= 2) {
          ...
          if (blocksWithTime.length > 0) {
            const avgTime =
              blocksWithTime.reduce((sum, block) => sum + block.blockTime, 0) /
              blocksWithTime.length;
            ...
            for (let i = 0; i < recentBlocks.length - 1; i++) {
              ...
              if (current.timestamp && next.timestamp) {
                ...
                if (timeDiff > 0 && timeDiff < 300) {
                  // Reasonable block time (< 5 minutes)
    ...
    300000
  ); // 5 minute cache for block time (for t4g.small)
  ...
  const totalTransactions = await getCachedData(
    'totalTransactions',
    async () => {
      try {
    ...
    120000
  ); // 2 minute cache for transaction count
