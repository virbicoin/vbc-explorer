  ...
  const tokens = useMemo(() => {
    ...
    for (let i = 0; i < userTokenAddresses.length; i++) {
      ...
      if (result.status === 'success' && result.result) {
        ...
        const isPaused =
          pauseResult?.status === 'success' ? (pauseResult.result as boolean) : false;
...
function MyTokenCard({ token }: { token: TokenInfo }) {
  ...
  const createdTime =
    ...
    ' ' +
    dateObj.toLocaleTimeString(undefined, { timeZoneName: 'short' }).split(' ').pop();
    ...
    <div className="bg-gray-800/50 rounded-xl p-4">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div
            className={`w-14 h-14 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white font-bold text-xl ${token.logoUrl ? 'hidden' : ''}`}
          >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {isVerified && (
                <span
                  className="px-1.5 py-0.5 bg-green-500/20 text-green-400 text-xs font-semibold rounded flex items-center gap-1"
      ...
      <div className="grid grid-cols-4 gap-4 mt-4 pt-4 border-t border-gray-700">
        ...
        <div className="flex justify-end items-center gap-1">
          <Link
            href={`/launchpad/token/${token.address}`}
            className="px-3 py-1.5 bg-purple-500/20 hover:bg-purple-500/30 rounded-lg text-xs text-purple-400 transition-colors font-medium"
            title="View Details"
          >
            👁️ Manage
          </Link>
