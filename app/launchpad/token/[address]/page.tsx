...
function TokenDetailContent() {
  ...
  const { isLoading: isConfirmingPause, isSuccess: isPauseConfirmed } =
    useWaitForTransactionReceipt({ hash: pauseHash });

  const {
    writeContract: writeMetadata,
    data: metadataHash,
    isPending: isUpdating,
  } = useWriteContract();
  const { isLoading: isConfirmingMetadata, isSuccess: isMetadataConfirmed } =
    useWaitForTransactionReceipt({ hash: metadataHash });
  ...
  useEffect(() => {
    if (isPauseConfirmed) refetchPaused();
  }, [isPauseConfirmed, refetchPaused]);
  useEffect(() => {
  ...
  const fetchHolders = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/tokens/${tokenAddress}?holdersPage=${holdersPage}&holdersLimit=${ITEMS_PER_PAGE}`
      );
      if (res.ok) {
  ...
  const fetchTransfers = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/tokens/${tokenAddress}?transfersPage=${transfersPage}&transfersLimit=${ITEMS_PER_PAGE}`
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const handleTogglePause = () => {
    ...
    writePause({
      address: tokenAddress as Address,
      abi: LaunchpadTokenV2ABI,
      functionName: token?.isPaused ? 'unpause' : 'pause',
      args: [],
    });
  };
  ...
  const handleTransfer = async () => {
    ...
    setActionError(null);
    if (!isAddress(actionRecipient)) {
      setActionError('Invalid recipient address');
      return;
    }
    try {
      ...
      if (amountToSend > balance) {
        setActionError(
          `Insufficient balance. You have ${formatUnits(balance, token.decimals)} ${token.symbol}`
        );
        return;
      }
      writeAction({
        ...
        args: [actionRecipient as Address, amountToSend],
      });
    } catch {
  ...
  const handleApprove = async () => {
    ...
    setActionError(null);
    if (!isAddress(actionSpender)) {
      setActionError('Invalid spender address');
      return;
    }
    try {
      ...
      writeAction({
        ...
        args: [actionSpender as Address, amountToApprove],
      });
    } catch {
      setActionError(`Invalid amount format: ${actionAmount}`);
    }
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-gray-800/90 rounded-3xl p-8 shadow-xl border border-gray-700/50 text-center">
            ...
            <p className="text-gray-400 mb-6">
              This token does not exist or was not created through TokenFactory V2.
            </p>
            <Link
              href="/launchpad"
          ...
          <div className="flex items-start gap-6 mb-8">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                {isVerified && (
                  <span
                    className="px-2 py-0.5 bg-green-500/20 text-green-400 text-xs font-semibold rounded-lg flex items-center gap-1"
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1.5 hover:bg-gray-700 rounded-lg transition-colors flex items-center gap-1"
            ...
            <div className="flex flex-col gap-2">
              {token.website && (
                <a
                  ...
                  className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-xl text-white text-sm transition-colors flex items-center gap-2"
                >
              ...
              <Link
                ...
                className="px-4 py-2 bg-purple-500/20 hover:bg-purple-500/30 rounded-xl text-purple-400 text-sm transition-colors text-center"
              >
                View on Explorer
              </Link>
            </div>
          ...
          <div className="flex gap-1 mb-6 bg-gray-800/50 rounded-xl p-1">
            {(['overview', 'holders', 'transfers'] as TabType[]).map((tab) => (
              <button
                ...
                className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-colors ${activeTab === tab ? 'bg-purple-500 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700/50'}`}
              >
                {tab === 'overview'
                  ? '📊 Overview'
                  : tab === 'holders'
                    ? `👥 Holders ${totalHolders > 0 ? `(${totalHolders})` : ''}`
                    : `📜 Transfers ${totalTransfers > 0 ? `(${totalTransfers})` : ''}`}
              </button>
            ))}
          </div>
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div className="bg-gray-800/50 rounded-xl p-4">
                  ...
                  <div className="text-lg text-white font-semibold">
                    {Number(formattedSupply).toLocaleString(undefined, {
                ...
                <div className="bg-gray-800/50 rounded-xl p-4">
                  ...
                  <div className="text-lg text-white font-semibold">{token.decimals}</div>
                </div>
                <div className="bg-gray-800/50 rounded-xl p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div className="bg-gray-800/50 rounded-xl p-4">
                  ...
                  <Link
                    href={`/address/${token.creator}`}
                    className="text-purple-400 hover:text-purple-300 font-mono text-sm"
                  >
                    {token.creator.slice(0, 14)}...{token.creator.slice(-10)}
                  </Link>
                </div>
                <div className="bg-gray-800/50 rounded-xl p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <Link
                      ...
                      className="text-purple-400 hover:text-purple-300 font-mono text-sm"
                    >
                      {token.owner.slice(0, 14)}...{token.owner.slice(-10)}
                    </Link>
                    {isOwner && (
              ...
              {isOwner && (
                <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-xl p-6 border border-purple-500/30 mb-6">
                  ...
                  <p className="text-gray-400 text-sm mb-4">
                    As the owner of this token, you have access to the following management
                    features:
                  </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <button
                      ...
                      className="bg-gray-800/50 hover:bg-gray-700/50 rounded-xl p-4 text-left transition-colors group border border-transparent hover:border-blue-500/30"
                    >
                      ...
                      <p className="text-gray-500 text-xs">
                        Send tokens to another wallet address. Useful for distribution, payments, or
                        moving funds.
                      </p>
                    </button>
                    ...
                    <button
                      ...
                      className="bg-gray-800/50 hover:bg-gray-700/50 rounded-xl p-4 text-left transition-colors group border border-transparent hover:border-green-500/30"
                    >
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-xl">✅</span>
                      ...
                      <p className="text-gray-500 text-xs">
                        Allow a contract (DEX, staking, etc.) to spend your tokens. Required for
                        trading on DEXes.
                      </p>
                    </button>
                    ...
                    <button
                      ...
                      className="bg-gray-800/50 hover:bg-gray-700/50 rounded-xl p-4 text-left transition-colors group border border-transparent hover:border-red-500/30"
                    >
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-xl">🔥</span>
                      ...
                      <p className="text-gray-500 text-xs">
                        Permanently destroy tokens to reduce supply. This action is irreversible.
                      </p>
                    </button>
                    ...
                    <button
                      ...
                      className={`bg-gray-800/50 hover:bg-gray-700/50 rounded-xl p-4 text-left transition-colors group border border-transparent disabled:opacity-50 ${token.isPaused ? 'hover:border-green-500/30' : 'hover:border-yellow-500/30'}`}
                    >
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-xl">
                          {isPausing || isConfirmingPause ? '⏳' : token.isPaused ? '▶️' : '⏸️'}
                        </span>
                        <span
                          className={`font-semibold ${token.isPaused ? 'text-green-400 group-hover:text-green-300' : 'text-yellow-400 group-hover:text-yellow-300'}`}
                        >
                      ...
                      <p className="text-gray-500 text-xs">
                        {token.isPaused
                          ? 'Resume all token transfers. Users will be able to send and receive tokens again.'
                          : 'Temporarily halt all token transfers. Useful for emergencies or maintenance.'}
                      </p>
                    </button>
                    ...
                    <button
                      ...
                      className="bg-gray-800/50 hover:bg-gray-700/50 rounded-xl p-4 text-left transition-colors group border border-transparent hover:border-purple-500/30"
                    >
                      <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-gray-500 text-xs">
                        Update token logo, description, and website URL. Helps users identify your
                        token.
                      </p>
                    </button>
                    ...
                    {/* Verify Contract */}
                    <Link
                      href={`/contract/verify?address=${tokenAddress}&contractName=${encodeURIComponent(token.name)}&isLaunchpadToken=true`}
                      className="bg-gray-800/50 hover:bg-gray-700/50 rounded-xl p-4 text-left transition-colors group border border-transparent hover:border-blue-500/30"
                    >
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-xl">🛡️</span>
                      ...
                      <p className="text-gray-500 text-xs">
                        Verify and publish your contract source code. Makes your token more
                        trustworthy.
                      </p>
                    </Link>
                  </div>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-gray-700">
                          <th className="text-left py-3 px-4 text-xs text-gray-500 font-medium">
                            Rank
                          </th>
                          <th className="text-left py-3 px-4 text-xs text-gray-500 font-medium">
                            Address
                          </th>
                          <th className="text-right py-3 px-4 text-xs text-gray-500 font-medium">
                            Balance
                          </th>
                          <th className="text-right py-3 px-4 text-xs text-gray-500 font-medium">
                            %
                        ...
                        {holders.map((holder, index) => (
                          ...
                          >
                            <td className="py-3 px-4 text-sm text-gray-400">
                              {(holdersPage - 1) * ITEMS_PER_PAGE + index + 1}
                            </td>
                            ...
                            <td className="py-3 px-4 text-right text-sm text-gray-400">
                              {parseFloat(String(holder.percentage || 0)).toFixed(2)}%
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  {totalHoldersPages > 1 && (
                    <div className="flex items-center justify-center gap-2 mt-6">
                      <button
                        ...
                        className="px-3 py-1 bg-gray-700 hover:bg-gray-600 rounded-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        ←
                      </button>
                      ...
                      <button
                        onClick={() => setHoldersPage((p) => Math.min(totalHoldersPages, p + 1))}
                ...
                <>
                  <div className="space-y-2">
                    {transfers.map((tx) => {
                      ...
                      const txTimeStr = txDate
                        ? `${txDate.toLocaleString()} ${txDate.toLocaleTimeString(undefined, { timeZoneName: 'short' }).split(' ').pop()}`
                        : `Block #${tx.blockNumber}`;
                      return (
                        <div
                          key={tx.hash}
                          className="bg-gray-800/50 rounded-xl p-4 hover:bg-gray-700/30 transition-colors"
                        >
                  ...
                  {totalTransfersPages > 1 && (
                    <div className="flex items-center justify-center gap-2 mt-6">
                      ...
                      <span className="text-sm text-gray-400">
                        {transfersPage} / {totalTransfersPages}
                      </span>
                      <button
                        onClick={() =>
                          setTransfersPage((p) => Math.min(totalTransfersPages, p + 1))
                        }
          ...
          <div className="bg-gray-800 rounded-2xl p-6 max-w-lg w-full border border-gray-700">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-3 bg-gray-700/50 border border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"
                />
              </div>
              <div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                disabled={isUpdating || isConfirmingMetadata}
                className="flex-1 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold rounded-xl hover:opacity-90 disabled:opacity-50 transition-opacity flex items-center justify-center gap-2"
              >
                {isUpdating || isConfirmingMetadata ? (
                  <>
          ...
          <div className="bg-gray-800 rounded-2xl p-6 max-w-md w-full border border-gray-700">
            ...
            {isActionConfirmed ? (
              <div className="text-center py-6">
                ...
                <p className="text-gray-400 text-sm mb-4">
                  {actionAmount} {token.symbol} sent to {actionRecipient.slice(0, 10)}...
                </p>
                <button
                  onClick={closeActionModal}
              ...
              <>
                <div className="mb-4">
                  ...
                  <input
                    ...
                    placeholder="0x..."
                    className="w-full px-4 py-3 bg-gray-700/50 border border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm"
                  />
                </div>
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="0.0"
                      className="w-full px-4 py-3 pr-20 bg-gray-700/50 border border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                    <button
                      onClick={handleMaxAmount}
                      className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1 text-xs text-blue-400 hover:text-blue-300 bg-blue-500/20 hover:bg-blue-500/30 rounded-lg transition-colors"
                    >
                      MAX
                    </button>
                  </div>
                ...
                <button
                  ...
                  disabled={
                    !actionAmount || !actionRecipient || isActionPending || isActionConfirming
                  }
                  className="w-full py-3 bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-semibold rounded-xl hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity flex items-center justify-center gap-2"
                >
                  {isActionPending || isActionConfirming ? (
                    <>
                      ...
                      {isActionPending ? 'Confirming...' : 'Processing...'}
                    </>
                  ) : (
                    <>📤 Send Tokens</>
                  )}
          ...
          <div className="bg-gray-800 rounded-2xl p-6 max-w-md w-full border border-gray-700">
            ...
            {isActionConfirmed ? (
              <div className="text-center py-6">
                ...
                <h4 className="text-lg font-semibold text-white mb-2">Success!</h4>
                <p className="text-gray-400 text-sm mb-4">
                  Approval granted for {actionSpender.slice(0, 10)}...
                </p>
                <button
                  onClick={closeActionModal}
                  className="px-6 py-2 bg-gray-700 hover:bg-gray-600 rounded-xl text-white transition-colors"
              ...
              <>
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 mb-4">
                  <p className="text-blue-400 text-sm">
                    ℹ️ Approval allows a contract (like DEX) to spend your tokens on your behalf.
                  </p>
                </div>
                <div className="mb-4">
                  ...
                  <input
                    type="text"
                    value={actionSpender}
                    onChange={(e) => setActionSpender(e.target.value)}
                    placeholder="0x... (DEX Router, Staking Contract, etc.)"
                    className="w-full px-4 py-3 bg-gray-700/50 border border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500 font-mono text-sm"
                  />
                </div>
                <div className="mb-4">
                  <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => setActionAmount(e.target.value)}
                      placeholder="0.0"
                      className="w-full px-4 py-3 pr-32 bg-gray-700/50 border border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500"
                    />
                    <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-1">
                      <button
                        onClick={handleMaxAmount}
                        className="px-2 py-1 text-xs text-green-400 hover:text-green-300 bg-green-500/20 hover:bg-green-500/30 rounded-lg transition-colors"
                      >
                        MAX
                      </button>
                      <button
                        onClick={handleUnlimitedApprove}
                        className="px-2 py-1 text-xs text-purple-400 hover:text-purple-300 bg-purple-500/20 hover:bg-purple-500/30 rounded-lg transition-colors"
                      >
                        ∞
                      </button>
                    </div>
                ...
                <button
                  ...
                  disabled={
                    !actionAmount || !actionSpender || isActionPending || isActionConfirming
                  }
                  className="w-full py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white font-semibold rounded-xl hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity flex items-center justify-center gap-2"
                >
                  {isActionPending || isActionConfirming ? (
                    <>
                      <svg className="animate-spin h-5 w-5" viewBox="0 0 24 24">
          ...
          <div className="bg-gray-800 rounded-2xl p-6 max-w-md w-full border border-gray-700">
            ...
            {isActionConfirmed ? (
              <div className="text-center py-6">
                ...
                <h4 className="text-lg font-semibold text-white mb-2">Success!</h4>
                <p className="text-gray-400 text-sm mb-4">
                  {actionAmount} {token.symbol} has been burned.
                </p>
                <button
                  onClick={closeActionModal}
                  className="px-6 py-2 bg-gray-700 hover:bg-gray-600 rounded-xl text-white transition-colors"
              ...
              <>
                <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 mb-4">
                  <p className="text-red-400 text-sm">
                    ⚠️ Warning: Burning tokens is irreversible. The burned tokens will be
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => setActionAmount(e.target.value)}
                      placeholder="0.0"
                      className="w-full px-4 py-3 pr-20 bg-gray-700/50 border border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-red-500"
                    />
                    <button
                      onClick={handleMaxAmount}
                ...
                <button
                  onClick={handleBurn}
                  disabled={!actionAmount || isActionPending || isActionConfirming}
