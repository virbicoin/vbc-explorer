            ...
            <div className="flex items-center gap-1">
              ...
              <span
                className={`text-xs font-medium ${
                  isLive ? 'text-green-400 animate-pulse' : 'text-red-400'
                }`}
              >
                {isLive ? 'LIVE' : 'OFFLINE'}
              </span>
...
const TransactionList = ({
  ...
  const getTransactionTypeBadge = (type?: string, action?: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-xs font-medium ${config.bg} ${config.text}`}
      >
  ...
  <div className="bg-gray-800 rounded-lg border border-gray-700 p-6 h-full flex flex-col">
    ...
    <div className="space-y-2 flex-1">
      {[...Array(5)].map((_, i) => (
        <div
          key={i}
          className="flex justify-between items-center p-2.5 bg-gray-700/50 rounded border border-gray-600/50 animate-pulse"
        >
      ...
      try {
        ...
        if (isInitialLoad) {
          ...
          if (newTopBlock && lastTopBlock > 0 && newTopBlock > lastTopBlock) {
            ...
            const newBlocks =
              realtimeData.blocks?.filter((block: Block) => block.number > lastTopBlock) || [];
        ...
        if (
          !isInitialLoad &&
          newTopTransactionHash &&
          lastTopTransactionHash &&
          newTopTransactionHash !== lastTopTransactionHash
        ) {
          setNewTransactionHashes(new Set());
          const newTransactions = transactionsData.filter(
            (tx: Transaction) => tx.hash !== lastTopTransactionHash
          );
          const newTransactionHashesSet = new Set<string>(
            newTransactions.map((tx: Transaction) => tx.hash)
          );
          ...
          {isLoading ? (
            ...
            <>
              ...
              <SummaryCard
                ...
                value={
                  stats.lastBlockTimestamp && stats.lastBlockTimestamp > 0
                    ? (() => {
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
          {isLoading ? (
            ...
            <>
              <BlockList
                blocks={blocks}
                newBlockNumbers={newBlockNumbers}
                now={now}
                config={config}
              />
              <TransactionList
