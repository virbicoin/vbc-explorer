  ...
  try {
    ...
    if (!rateLimit.allowed) {
      return NextResponse.json(
        {
          error: 'Rate limit exceeded. Contract verification is limited to prevent abuse.',
    ...
    if (compiledOutput.errors) {
      ...
      const warnings = compiledOutput.errors.filter(
        (error: { severity: string }) => error.severity === 'warning'
      );
      ...
      return NextResponse.json(
        {
          ...
          message:
            'The source code could not be compiled successfully. Please check the syntax and try again.',
          debug: {
            detectedContractName,
    ...
    if (
      compiledOutput.contracts[detectedContractName] &&
      compiledOutput.contracts[detectedContractName][detectedContractName]
    ) {
    ...
    const removeMetadata = (bytecode: string): string => {
      ...
      for (let i = cleaned.length - 100; i < cleaned.length - 4; i++) {
        if (
          (cleaned.substring(i, i + 2) === 'a2' || cleaned.substring(i, i + 2) === 'a1') &&
    ...
    console.log(
      'Clean onchain bytecode (first 100 chars):',
      cleanOnchainBytecode.substring(0, 100)
    );
    console.log(
      'Clean compiled bytecode (first 100 chars):',
      cleanCompiledBytecode.substring(0, 100)
    );
    ...
    const isVerified6 = lengthRatio > 0.9 && similarity > 0.9;
