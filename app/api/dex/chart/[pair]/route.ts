...
export async function GET(request: NextRequest, { params }: { params: Promise<{ pair: string }> }) {
  try {
    ...
    if (!web3.utils.isAddress(pairAddress)) {
      return NextResponse.json({ success: false, error: 'Invalid pair address' }, { status: 400 });
    }
